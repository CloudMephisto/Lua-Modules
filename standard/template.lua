...
function Template.storeReturnValue(value, namespace)
...
--[[
Retrieves all values emplaced by Template.storeReturnValue.
]]
...
--[[
Variant of Template.storeReturnValue suitable for use by #invoke. Stores the
arguments of a template call in a place to be later retrieved by
Template.retrieveReturnValues. {{#invoke:Template|storeArgs}} has the same
interface as {{#invoke:Json|fromArgs}}.

Usage:

{{#invoke:Template|storeArgs|foo=3|namespace=Magpie}}

will make {foo = '3'} available for retrival via Template.retrieveReturnValues('Magpie') .

]]
function Template.storeArgs(frame)
	local args = Arguments.getArgs(frame)
	local namespace = args.namespace
	args.namespace = nil
	return Template.storeReturnValue(args, namespace)
end
