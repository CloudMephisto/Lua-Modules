...
function Match.storeMatchGroup(matchRecords, options)
	...
	matchRecords = Array.map(matchRecords, function(matchRecord)
		...
		Match.prepareRecordsForStore(records)
...
--[[
Final processing of records before stored to LPDB.
]]
function Match.prepareRecordsForStore(records)
	Match.prepareMatchRecordForStore(records.matchRecord)
	for opponentIndex, opponentRecord in ipairs(records.opponentRecords) do
...
function Match.prepareMatchRecordForStore(match)
