...
function copyPaste._getBracketData(templateid)
	...
	local bracketDatas = Array.map(matches, function(match)
	...
	local function sortKey(bracketData)
		...
		if bracketData.matchKey == 'RxMTP' or bracketData.matchKey == 'RxMBR' then
			...
			local finalBracketData = Array.find(bracketDatas, function(bracketData_)
	...
	Array.sortInPlaceBy(bracketDatas, sortKey)
...
function copyPaste.bracket(frame, args)
	...
	local bracketDatas = copyPaste._getBracketData(templateid)

	for index, bracketData in ipairs(bracketDatas) do
